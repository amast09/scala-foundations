...
class ValidationExercisesTest extends AnyFunSuite with ScalaCheckDrivenPropertyChecks {
  ...
  test("validateUser example") {
    ...
    assert(validateUser("bob_2167", "UK") == FieldError(FieldIds.countryOfResidence, NEL(InvalidFormat("UK"))).invalid)
    assert(validateUser("bo", "FRA") == FieldError(FieldIds.username, NEL(TooSmall(2))).invalid)
    assert(
      validateUser("b!", "UK") == Validation.Invalid(
        NEL(
