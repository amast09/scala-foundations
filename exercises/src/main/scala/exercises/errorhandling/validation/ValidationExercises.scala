...
object ValidationExercises {
  ...
  object FormError {
    ...
    case class InvalidCharacters(char: List[Char]) extends FormError // TODO: Convert List => NEL
  }
  ...
  def validateCountry(countryCode: String): Validation[FormError, Country] = {
    ...
    maybeCountry.toValid(countryCodeIsValidFormat(countryCode) match {
  ...
  def checkUsernameSize(username: String): Validation[TooSmall, Unit] = {
    ...
    Validation.cond(usernameLength >= 5, (), TooSmall(usernameLength))
  }
  ...
  def checkUsernameCharacters(username: String): Validation[InvalidCharacters, Unit] =
    username.toList.filter(c => !isValidUsernameCharacter(c)).distinct match {
      ...
      case invalidCharactersInUsername => Validation.invalid(InvalidCharacters(invalidCharactersInUsername))
    }
  ...
  def field[A](fieldId: String)(validation: Validation[FormError, A]): Validation[FieldError, A] =
    validation.mapErrorAll(errors => NEL(FieldError(fieldId, errors)))
