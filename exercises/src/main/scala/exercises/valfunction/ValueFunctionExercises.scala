...
object ValueFunctionExercises {
  ...
  val VALID_USERNAME_CHARACTERS = (('a' to 'z') ++ ('A' to 'Z') ++ ('0' to '9') ++ List('-', '_')).toSet

  def isValidUsernameCharacter(char: Char): Boolean =
    VALID_USERNAME_CHARACTERS.contains(char)
  ...
  case class Point(x: Int, y: Int, z: Int) {
    ...
    def forAll(predicate: Int => Boolean): Boolean = List(x, y, z).forall(predicate)
  }
}
