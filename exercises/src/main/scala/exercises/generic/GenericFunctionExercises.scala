...
object GenericFunctionExercises {
  ...
  lazy val decoded: Pair[String] = secret.map(_.map(_.toChar).reverse.mkString).swap
  ...
  case class Predicate[A](eval: A => Boolean) {
    ...
    def contramap[To](extractValue: To => A): Predicate[To] =
      Predicate(v => eval(extractValue(v)))
  }

  object Predicate {
    def True[T]: Predicate[T]  = Predicate(_ => true)
    def False[T]: Predicate[T] = Predicate(_ => false)
  }
  ...
  def by[To](extractValue: User => To)(predicate: Predicate[To]): Predicate[User] =
    predicate.contramap(extractValue)

  def isGreaterThan(min: Int): Predicate[Int] =
    Predicate(n => n > min)
  def isLongerThan(n: Int): Predicate[String] =
    isGreaterThan(n).contramap(_.length)
  val isCapitalized: Predicate[String] =
    Predicate(s => s.capitalize == s)

  lazy val isValidUser: Predicate[User] =
    by(_.age)(isGreaterThan(18)) &&
      by(_.name)(isLongerThan(2) && isCapitalized)
  ...
  trait JsonDecoder[A] {
    ...
    def map[To](update: A => To): JsonDecoder[To] = { (json: Json) =>
      update(decode(json))
    }
    def orElse(fallbackDecoder: JsonDecoder[A]): JsonDecoder[A] = { (json: Json) =>
      Try(decode(json)).getOrElse(fallbackDecoder.decode(json))
    }
  }
  ...
  val longDecoder: JsonDecoder[Long] = _.toLong
  ...
  val intDecoderSAM: JsonDecoder[Int] = _.toInt
  ...
  lazy val userIdDecoder: JsonDecoder[UserId] =
    intDecoder.map(UserId) //UserId(intDecoder.decode(id))
  ...
  def firstDecoder[To](decoders: List[JsonDecoder[To]]): JsonDecoder[To] =
    json => {
      decoders.find(d => Try(d.decode(json)).isSuccess).get.decode(json)
    }

  def epochDayDecoder: JsonDecoder[LocalDate] =
  ...
  lazy val weirdLocalDateDecoder: JsonDecoder[LocalDate] =
    firstDecoder(List(localDateDecoder, epochDayDecoder))
  ...
  def optionDecoder[A](decoder: JsonDecoder[A]): JsonDecoder[Option[A]] =
    s => Try(decoder.decode(s)).toOption
  ...
  // Answer: Update the `decode` method to return an Option[A] instead of an `A`
}
