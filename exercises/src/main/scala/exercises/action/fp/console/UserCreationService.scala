...
class UserCreationService(console: Console, clock: Clock) {
  ...
  val readDateOfBirth: IO[LocalDate] = {
    val parseDateOfBirthWithErrorMsg = (l: String) =>
      parseDateOfBirth(l).onError(_ =>
    ...
    writeLine("What's your date of birth? [dd-mm-yyyy]")
      .andThen(readLine)
      .flatMap(parseDateOfBirthWithErrorMsg)
  }
  ...
  val readSubscribeToMailingList: IO[Boolean] = {
    val parseLineToBooleanWithErrorMsg = (l: String) =>
      parseLineToBoolean(l).onError(_ => writeLine("""Incorrect format, enter "Y" for Yes or "N" for "No""""))

    writeLine("Would you like to subscribe to our mailing list? [Y/N]")
      .andThen(readLine)
      .flatMap(parseLineToBooleanWithErrorMsg)
  }
