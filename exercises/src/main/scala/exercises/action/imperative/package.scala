...
package object imperative {
  ...
  def retry[A](maxAttempt: Int)(action: => A): A =
    if (maxAttempt < 1) throw new IllegalArgumentException("Invalid `maxAttempt` parameter, `maxAttempt` must be > 0")
    else if (maxAttempt == 1) action
    else {
      Try(action) match {
        case Failure(exception) => retry(maxAttempt - 1)(action)
  ...
  //////////////////////////////////////////////
  // TODO: Bonus questions (not covered by the video)
  //////////////////////////////////////////////
